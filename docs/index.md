...
Sticky sidebar smart enough to handle sidebar when it's taller than viewport. You don't have to worry about content of sidebar it will scroll sidebar independently up and down.
...
You can install sticky sidebar jquery plugin from Bowser, NPM or just simply download it from <a href="#">Github</a> than put ``sticky-sidebar.js`` file in your project folder.
...
The biggest cause of scrolling jank is ``onScroll`` has a lot of work. But in this plugin we cached all dimensions and all animate functions inside `requestAnimationFrame` function, as well as adding `will-change: transform` and work with `translate(Y, X)` instead of `top: Y; Left: X;` increases performance significantly, We built Sticky sidebar plugin prevents repainting and reflow to make it very smooth as much as possible.
